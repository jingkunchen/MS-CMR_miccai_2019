...
def main(train_imgs_np_file,
    ...
    print("train_imgs, train_masks, num_classes:", train_imgs.shape, train_masks.shape, num_classes)
        ...
        for epoch in range(0, total_epochs):
            ...
            if eval_per_epoch and epoch % 10 == 0:
                ...
                sitk.WriteImage(
                    ...
                    output + '/' + test_imgs_np_file_1[0:6] + '_pretrained_unet_patch_gan_pred_epoch_' +
                    str(current_epoch) + '.nii.gz')
                sitk.WriteImage(
                    sitk.GetImageFromArray(orig_mask_2),
                    output + '/' + test_imgs_np_file_2[0:6] + '_pretrained_unet_patch_gan_pred_epoch_' +
                    str(current_epoch) + '.nii.gz')
    ...
    if use_patch_gan:
        ...
        for epoch in range(0, total_epochs):
            ...
            if eval_per_epoch and epoch % 10 == 0:
                ...
                sitk.WriteImage(
                    sitk.GetImageFromArray(orig_mask_1),
                    output + '/' + test_imgs_np_file_1[0:6] + '_patch_gan_pred_epoch_' +
                    str(current_epoch) + '.nii.gz')
                sitk.WriteImage(
                    sitk.GetImageFromArray(orig_mask_2),
                    output + '/' + test_imgs_np_file_2[0:6] + '_patch_gan_pred_epoch_' +
                    str(current_epoch) + '.nii.gz')
            ...
            if epoch % 2 == 0:
                gen_weights_path = os.path.join(
                    './weights/gen_weights_epoch_%s.h5' % (epoch))
                generator_nn.save_weights(gen_weights_path, overwrite=True)

                disc_weights_path = os.path.join(
                    './weights/disc_weights_epoch_%s.h5' % (epoch))
                discriminator.save_weights(disc_weights_path, overwrite=True)

                DCGAN_weights_path = os.path.join(
                    './weights/DCGAN_weights_epoch_%s.h5' % (epoch))
                dc_gan_nn.save_weights(DCGAN_weights_path, overwrite=True)
    ...
    if use_patch_gan_discrimator:
        sub_patch_dim = (56, 56)
        num_classes_discrimator = 5
        mask_shape_discrimator = (train_masks.shape[1], train_masks.shape[2], num_classes_discrimator)
        nb_patch_patches, patch_gan_dim = patch_utils.num_patches(
            output_img_dim=mask_shape_discrimator, sub_patch_dim=sub_patch_dim)
        print("nb_patch_patches:", nb_patch_patches)
        print("patch_gan_dim:", patch_gan_dim)
        discriminator = PatchGanDiscriminator(output_img_dim=mask_shape_discrimator,
                                              patch_dim=patch_gan_dim,
                                              nb_patches=nb_patch_patches)
        discriminator.summary()
        # disable training while we put it through the GAN
    ...
    # pretrained unet CNN model
        ...
        while current_epoch <= total_epochs:
            ...
            if eval_per_epoch and current_epoch % 10 == 0:
                ...
                sitk.WriteImage(
                    sitk.GetImageFromArray(orig_mask_1),
                    output + '/' + test_imgs_np_file_1[0:6] + '_pretrained_unet_cnn_pred_epoch_' +
                    str(current_epoch) + '.nii.gz')
                sitk.WriteImage(
                    sitk.GetImageFromArray(orig_mask_2),
                    output + '/' + test_imgs_np_file_2[0:6] + '_pretrained_unet_cnn_epoch_' +
                    str(current_epoch) + '.nii.gz')
    ...
    # CNN model
        ...
        while current_epoch <= total_epochs:
            ...
            if eval_per_epoch and current_epoch % 10 == 0:
                ...
                sitk.WriteImage(
                    sitk.GetImageFromArray(orig_mask_1),
                    output + '/' + test_imgs_np_file_1[0:6] + '_cnn_pred_epoch_' +
                    str(current_epoch) + '.nii.gz')
                sitk.WriteImage(
                    sitk.GetImageFromArray(orig_mask_2),
                    output + '/' + test_imgs_np_file_2[0:6] + '_cnn_pred_epoch_' +
                    str(current_epoch) + '.nii.gz')
